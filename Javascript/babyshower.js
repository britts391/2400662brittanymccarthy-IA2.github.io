...
function addToCart(themeName, price, textInputId, errorId) {
    ...
    if (!username) {// check if current user is empty if so it means user is not login
        // If no user session is found, show alert and stop
        alert("User not logged in.");
        return;
    }
  // Get references to the user input and error message area
    // Using getElementById() to retrieve specific elements from the DOM
    var babyTextInput = document.getElementById(textInputId);
    var errorMsg = document.getElementById(errorId);//document is  the browser’s representation of html which is used to access and modify elements such as getElementById.
    var babyText = babyTextInput.value.trim(); // trim is use to remove extra spaces

    //  Input Validation — Ensure backdrop text is entered
    // (IA#2 2c - Form Validation)
    if (!babyText) {
        // If the field is empty, show an error message dynamically
        errorMsg.textContent = "Please enter backdrop text.";//textContent  is a built in property to set or get the text inside that element.
    ...
    //  Retrieve existing cart data or create a new one
